...
export default function TeamManagement() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [usersRes, rolesRes, locationsRes] = await Promise.all([
        api.get('/api/users'),
        api.get('/api/roles'),
        api.get('/api/locations').catch(() => []) // Catch if locations API is missing or error
      ]);
      setUsers(usersRes);
      setRoles(rolesRes);
  ...
  const handleUserSubmit = async (e) => {
    ...
    try {
      if (userForm.id) {
        await api.put(`/api/users/${userForm.id}`, { name: userForm.name, role_id: userForm.role_id, location_id: userForm.location_id || null });
      } else {
  ...
  const toggleUserStatus = async (user) => {
    try {
      ...
      await api.put(`/api/users/${user.id}`, { name: user.name, role_id: user.role_id, status: newStatus });
      fetchData();
    } catch (err) {
  ...
  return (
    <div>
      <header className="dashboard-header" style={{ marginBottom: '24px' }}>
        <div>
          <h1 className="dashboard-title">Team & Roles</h1>
          <p className="dashboard-subtitle">Manage users, custom roles, and permissions across your business.</p>
        </div>
      </header>
                    ...
                    {users.map(u => (
                      <tr key={u.id}>
                        ...
                        <td className="text-right">
                          ...
                          <button className="btn-icon text-error hover-bg-error" onClick={() => handleDeleteUser(u.id)} title="Delete">🗑</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
