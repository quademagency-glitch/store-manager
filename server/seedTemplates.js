...
async function seed() {
  ...
  for (const business of businesses) {
    ...
    console.log(`Inserting templates for business: ${businessId}`);
    ...
    if (existing && existing.length > 0) {
      console.log(`Templates already exist for business ${businessId}. Skipping...`);
      continue;
    }
    ...
    if (error) {
      console.error(`Failed to insert templates for ${businessId}`, error);
    } else {
      console.log(`Successfully seeded templates for ${businessId}!`);
    }
  }
