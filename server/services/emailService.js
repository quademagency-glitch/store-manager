/**
 * Email Service — Resend-based transactional emails
 * Handles invoice sending, subscription notifications, and expiration warnings.
 */
...
function getResendClient() {
  if (!resend) {
    try {
      ...
      console.warn('Resend not available. Email sending will be simulated.');
      resend = null;
    }
  }
  return resend;
...
const PLATFORM_NAME = 'Store Manager';
const FROM_EMAIL = process.env.FROM_EMAIL || 'billing@resend.dev'; // Use verified domain in production
          ...
          <tr>
            <td style="background:#f8fafc;padding:24px 40px;border-top:1px solid #e2e8f0;">
              <p style="margin:0;color:#94a3b8;font-size:12px;text-align:center;">
                This invoice was generated by ${PLATFORM_NAME}. If you have questions, contact us at ${PLATFORM_ADMIN_EMAIL || 'support@storemanager.app'}.
              </p>
            </td>
          </tr>
...
async function sendInvoiceEmail(invoice, business, planName, recipientEmails = []) {
  ...
  if (recipients.length === 0) {
    console.warn(`No recipients for invoice ${invoice.invoice_number}`);
  ...
  if (!client) {
    console.log(`[EMAIL SIMULATED] Invoice ${invoice.invoice_number} to: ${recipients.join(', ')}`);
  ...
  try {
    ...
    console.log(`Invoice email sent: ${invoice.invoice_number} to ${recipients.join(', ')}`);
...
async function sendExpirationWarning(business, subscription, daysLeft) {
  ...
  if (!client) {
    console.log(`[EMAIL SIMULATED] Expiration warning for ${business.name} (${daysLeft} days left)`);
  ...
  try {
    const { error } = await client.emails.send({
      from: `${PLATFORM_NAME} <${FROM_EMAIL}>`,
      to: recipients,
      subject: `⚠️ Subscription expiring in ${daysLeft} day${daysLeft !== 1 ? 's' : ''} — ${business.name}`,
      html,
    });
...
async function sendSuspensionNotice(business) {
  ...
  if (!client) {
    console.log(`[EMAIL SIMULATED] Suspension notice for ${business.name}`);
    return { success: true, simulated: true };
  }
