...
try {
  ...
  console.warn('node-cron not installed. Subscription cron will not run.');
}
  ...
  try {
    ...
    for (const sub of expired) {
      ...
      if (sub.amount <= 0) {
        ...
        console.log(`[CRON] Auto-renewed free plan for business: ${sub.businesses?.name}`);
        continue;
      }
      ...
      console.log(`[CRON] Suspended business: ${sub.businesses?.name} (subscription expired)`);
    }
  } catch (err) {
    console.error('[CRON] Error processing expired subscriptions:', err);
  }
}
...
async function sendExpirationWarnings() {
  ...
  try {
    ...
    console.log(`[CRON] Found ${expiring.length} subscription(s) expiring within 3 days.`);
    ...
    for (const sub of expiring) {
      ...
      if (sub.businesses) {
        ...
        console.log(`[CRON] Sent expiration warning to ${sub.businesses.name} (${daysLeft} days left)`);
      }
    }
  } catch (err) {
    console.error('[CRON] Error sending expiration warnings:', err);
  }
}
...
async function runSubscriptionChecks() {
  ...
  console.log('[CRON] === Subscription checks complete ===');
}
...
function initSubscriptionCron() {
  if (!cron) {
    console.warn('[CRON] node-cron not available. Subscription cron disabled.');
    return;
  }
  ...
  console.log('✅ Subscription cron job initialized (runs daily at midnight GMT)');
