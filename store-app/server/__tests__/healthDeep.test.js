...
// Each case is a separate monitor. Keep the production six-per-minute limiter
// enabled without letting previous cases consume the next case's allowance.
let monitorId = 10;
const deepRequest = () => request(app).get('/api/health/deep')
  .set('X-Forwarded-For', `203.0.113.${monitorId}`);
...
describe('GET /api/health/deep', () => {
  ...
  it('reports 503 when Supabase is unreachable', async () => {
    ...
    // Change the dependency outcome on the existing app. Reloading the entire
    // server inside a 15-second test measures cold module loading too and can
    // time out before the health request starts on a busy development machine.
    const originalFrom = mockSupabase.from;
    mockSupabase.from = failing.from;
    _resetCache();
    try {
      const res = await deepRequest();
