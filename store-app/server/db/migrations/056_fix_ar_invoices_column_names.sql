-- ============================================
-- Migration 056: Fix ar_invoices column names
-- The live ar_invoices table was created with `amount`/`issue_date`
-- (copied from the ap_bills pattern), but every consumer — the
-- /api/ar/aging and /api/reports/ar-aging routes, the bulk-import
-- opening-balance committer, and the AR billing UI — was built against
-- `total_amount`/`issued_date` (per migration 044's documented intent).
-- Renaming the columns to match the much larger, already-deployed
-- application surface, rather than rewriting every consumer.
-- ============================================

ALTER TABLE public.ar_invoices RENAME COLUMN amount TO total_amount;
ALTER TABLE public.ar_invoices RENAME COLUMN issue_date TO issued_date;

-- record_ar_payment (migration 046) referenced the old `amount` column name.
