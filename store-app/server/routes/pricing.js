...
router.post('/preview', authGuard, permissionCheck('manage_products'), async (req, res) => {
  try {
    ...
    logger.error({ err: err }, 'Error previewing price update:');
...
router.put('/bulk-update', authGuard, permissionCheck('manage_products'), async (req, res) => {
  try {
    ...
    for (const p of products) {
      ...
      if (updateErr) {
        logger.error({ err: updateErr }, 'Failed to update price for ${p.sku}:');
        continue;
      }
    ...
    if (logEntries.length > 0) {
      ...
      if (logErr) {
        logger.error({ err: logErr }, 'Failed to insert price change log:');
      }
    }
    ...
    logger.error({ err: err }, 'Error in bulk price update:');
...
router.put('/update-cost', authGuard, permissionCheck('manage_products'), async (req, res) => {
  try {
    ...
    logger.error({ err: err }, 'Error updating cost price:');
...
router.get('/history', authGuard, permissionCheck('manage_products'), async (req, res) => {
  try {
    ...
    logger.error({ err: err }, 'Error fetching price history:');
...
router.get('/categories', authGuard, async (req, res) => {
  try {
    ...
    logger.error({ err: err }, 'Error fetching categories:');
